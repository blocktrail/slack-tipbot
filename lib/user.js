...
User.prototype.getBalanceLine = function(cb) {
    ...
    self.getWallet(function(wallet) {
        wallet.getBalance(function(err, confirmed, unconfirmed) {
            ...
            cb(null, self.handle + " confirmed; " + blocktrail.toBTC(confirmed) + " " + self.tipbot.TICKER + " | unconfirmed; " + blocktrail.toBTC(unconfirmed) + " " + self.tipbot.TICKER);
        });
    });
};
...
User.prototype.tellDepositAddress = function(channel) {
    ...
    self.getWallet(function(wallet) {
        wallet.getNewAddress(function(err, address) {
            ...
            channel.send(self.handle + " you can deposit to; " + address);
        });
    });
};
    ...
    self.getWallet(function(wallet) {
        wallet.getBalance(function(err, confirmed, unconfirmed) {
            if (err) {
                ...
                var pay = {};
                pay[address] = value;
                wallet.pay(pay, null, false, true, self.tipbot.OPTIONS.FEE_STRATEGY || blocktrail.Wallet.FEE_STRATEGY_OPTIMAL, function(err, txHash) {
                    if (err) {
                        console.log(err);
                        channel.send(err.message);
                        return;
                    ...
                    channel.send("Withdrawl of " + blocktrail.toBTC(value) + " " + self.tipbot.TICKER + " to " + address + " transaction; " + url);
                });
    ...
    async.parallel({
        ...
        user: function(cb) {
            user.getWallet(function(wallet) {
                wallet.getNewAddress(wallet.changeChain, function(err, address) {
                    cb(err, address);
                });
            });
        }
        ...
        self.getWallet(function(wallet) {
            ...
            wallet.pay(send, null, false, true, self.tipbot.OPTIONS.FEE_STRATEGY || blocktrail.Wallet.FEE_STRATEGY_OPTIMAL, function(err, txHash) {
                if (err) {
                    console.log(err);
                    channel.send(err.message);
                    return;
                ...
                var url = self.tipbot.explorerBaseUrl + "/tx/" + txHash;
                channel.send("Sent " + blocktrail.toBTC(value) + " " + self.tipbot.TICKER + " from " + self.handle + " to " + user.handle + " transaction; " + url);
            });
        });
    })
};
