...
User.prototype.getWallet = function(cb) {
    var self = this;

    if (!self.wallet) {
        debug("SLACKBOT-" + self.id, self.name);

        // @TODO: change this, add a secret or something!
        var walletIdentifier = "SLACK-" + self.name,
            walletPassphrase = "SLACK-" + self.id;
