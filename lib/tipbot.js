...
var TipBot = function(slack, BLOCKTRAIL_APIKEY, BLOCKTRAIL_APISECRET, SECRET, TESTNET, OPTIONS) {
    ...
    self.CURRENCY_REGEX = new RegExp('(\\d+\\.?\\d*) *(Satoshis?|BTC|' + self.CURRENCIES.join("|") + ')', "i");
...
TipBot.prototype.normalizeValue = function(inputValue, unit, cb) {
    ...
    if (currency != 'BTC') {
        self.client.price(function(err, result) {
            if (!result[currency]) {
                ...
                var text = value.toFixed(2) + " " + currency + " " +
                    "(" + blocktrail.toBTC(newValue) + " BTC at " + rate.toFixed(2) + " " + currency + " / BTC)";
...
TipBot.prototype.onMessage = function(channel, member, message) {
    ...
    if (message.match(/withdraw/i)) {
        amount = message.match(self.CURRENCY_REGEX);
