...
var TipBot = function(slack, BLOCKTRAIL_APIKEY, BLOCKTRAIL_APISECRET, TESTNET, OPTIONS) {
    var self = this;

    self.slack = slack;
    self.client = blocktrail({apiKey: BLOCKTRAIL_APIKEY, apiSecret: BLOCKTRAIL_APISECRET, network: "BTC", testnet: TESTNET});
...
TipBot.prototype.updateUserRegex = function() {
    ...
    var ids = _.reject(_.map(self.users, 'id'), function(id) { return id == self.slack.self.id; });
    var names = _.reject(_.map(self.users, 'name'), function(id) { return id == self.slack.self.name; });
...
TipBot.prototype.init = function() {
    ...
    request.get(url, function(err, response, body) {
        ...
        async.forEach(Object.keys(self.users), function(userId, cb) {
            ...
            debug('TipBot ready!');
            debug('We have the following known users; ', _.map(self.users, function(user) { return user.name; }).join(', '));
        });
    });
};
...
TipBot.prototype.onMessage = function(channel, member, message) {
    ...
    var userMatches = _.reject(message.match(self.userRegex), function(match) { return match == user.id || match == user.name; });

    // find real user objects
    userMatches = _.filter(_.map(userMatches, function(match) {
        // if it's an ID
        if (self.users[match]) {
    ...
    if (userMatches.length == 1) {
         ...
         * DEMAND
         */
        if (amount && message.match(/(ask|demand|deserve|receive|get|send ?me|give ?me|gimme|ow[en]?s me)/i)) {
    ...
    if (_.any(triggers, function(trigger) { return trigger.match(message, user, userMatches); })) {
