...
TipBot.prototype.updateUserFromMember = function(member) {
    var self = this;
...
TipBot.prototype.init = function() {
    ...
    request.get(url, function(err, response, body) {
        ...
        async.forEachLimit(Object.keys(self.users), 3, function(userId, cb) {
            ...
            debug('We created [%d] new wallets', newWalletCnt);
        });
    });
};
...
TipBot.prototype.onMessage = function(channel, member, message) {
     ...
     * ALL BALANCES
     */
    debug('all?', message.match(/(all|every(one)?s?) ?balance/i));
     ...
     * BALANCE
     */
    debug('balance?', message.match(/balance/i));
