...
var TipBot = function(slack, BLOCKTRAIL_APIKEY, BLOCKTRAIL_APISECRET, SECRET, TESTNET, NETWORK, TICKER, OPTIONS) {
    ...
    if (NETWORK === 'BCC') {
        if (TESTNET) {
            self.explorerBaseUrl = "https://www.blocktrail.com/tBCC";
        } else {
            self.explorerBaseUrl = "https://bch.btc.com";
        }
    } else {
        if (TESTNET) {
            self.explorerBaseUrl = "https://tchain.btc.com";
        } else {
            self.explorerBaseUrl = "https://btc.com";
        }
    }
...
TipBot.prototype.onMessage = function(channel, member, message) {
    ...
    if (message.match(/withdraw/i)) {
        ...
        self.normalizeValue(amount[1], amount[3], function(value, rate, originalCurrency, originalValue, valueText) {
            if (rate === false) {
    ...
    if (message.match(/(convert|rate|to)/i)) {
        ...
        if (!currencies || currencies.length < 2) {
            ...
            if (amount) {
                ...
                amount[3] = amount[3].toUpperCase();
                currency = _.without(currencies, amount[3]);
                    ...
                    self.normalizeValue(amount[1], amount[3], function(value, rate, originalCurrency, originalValue, valueText) {
                        if (rate === false) {
                            ...
                            if (currency === self.TICKER) {
                                ...
                                channel.send(blocktrail.toBTC(value) + " " + self.TICKER + " (" + rate.toFixed(2) + " " + amount[3] + " / " + self.TICKER + ")");
                            } else if (amount[3] === self.TICKER) {
                                self.getPriceRates(function(err, rates) {
                                ...
                                self.getPriceRates(function(err, rates) {
                                    ...
                                    if (!currencyRate) {
                                        ...
                                        channel.send(newValue.toFixed(2) + " " + currency + " (" + conversionRate.toFixed(2) + " " + currency + " / " + amount[3] + ")");
                                    }
                                });
                            }
                        }
